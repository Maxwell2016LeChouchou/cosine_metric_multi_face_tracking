...
Histograms of 8 bins are calcuated with magnitudes as weights. Each image is checked if its of 32x32 size, else its resized.
The code reads images in greyscale. The images are normalized for gamma, and then, for normal contrast. 
Each 32x32 image pixel matrix, is organized into 8x8 cells and tehn, histograms are calculated for each cell.
Then, a 4x4 matrix with 8 bins in each cell is obtained. This matrix is organized as 2x2 blocks(with 50% overlap) and normalized, 
by dividing with the magnitude of histogram bins'vector. A total of 9 blocks x 4 cells x 8 bins = 288 features. 
Finally, these features are extracted for each image and trained with an SVM. 
